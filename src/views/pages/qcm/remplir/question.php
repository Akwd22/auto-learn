...
function afficherQcmQuestion(QCM $qcm, QuestionQCM $question, TentativeQCM $tentative)
{
  $choix = function () use (&$question) {
    foreach ($question->getAllChoix() as $choix) {
      $idChoix = htmlspecialchars($choix->getId());
      $intitule = htmlspecialchars($choix->getIntitule());

      if ($question->getIsMultiple()) {
        createCheckbox('choix-' . $idChoix, 'choix-' . $idChoix, $intitule, 'm', '', '', '');
      } else {
        createRadio('choix-' . $idChoix, 'choix', $intitule, $idChoix, 'm', '', '');
      }
    }
  };
  ...
  $qcmTitre = htmlspecialchars($qcm->getTitre());
  $qcmTotalPoints = htmlspecialchars($qcm->getTotalPoints());

  $questionPoints = htmlspecialchars($question->getPoints());
  $questionIntitule = htmlspecialchars($question->getQuestion());

  $tentativePointsActuels = htmlspecialchars($tentative->getPointsActuels());
?>
  ...
  <body>
        ...
        <form method="POST">
          ...
          <div id="centerDiv">
            ...
            <div id="containerQuestion">
              ...
              <div>
                <h1 id="titleQcm"><?php echo $qcmTitre; ?></h1>
                <p id="nbPoint"><?php echo $questionPoints; ?> point(s)</p>
              </div>
              <p id="question"><?php echo $questionIntitule; ?></p>
              ...
              <div id="buttonDiv">
                <div id="leftButtonDiv">
                  ...
                  <p id="score">Score: <?php echo $tentativePointsActuels . "/" . $qcmTotalPoints; ?></p>
                </div>
    ...
    <script>
      <?php $p = (int)($tentative->getNumQuestionCourante() - 1) * 100 / $qcm->nbQuestions();
      echo "fillProgressBar('" . $p . "');"; ?>
    </script>
  </body>
