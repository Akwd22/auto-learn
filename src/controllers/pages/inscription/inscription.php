...
function showView()
{
  include("views/pages/inscription/inscription.php");
  exit;
}
...
function handleForm()
{
  ...
  if (isset($_POST['pseudo']) && isset($_POST['password']) && isset($_POST['email'])) {
    ...
    if ($user != null) {
      redirect("/inscription", "error", "Nom d'utilisateur déjà existant.");
    } elseif ($email != null) {
      redirect("/inscription", "error", "E-mail déjà existante.");
    } else {
